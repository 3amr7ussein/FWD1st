...
imageApi.get('/', function (req, res) {
    ...
    var fullPath = path.join(path.resolve('./'), 'assets', 'full', "".concat(fileName, ".jpg"));
    if (!width && !height) {
        ...
        var resizedPath = path.join(path.resolve('./'), 'assets', 'thumb', "".concat(fileName).concat(width).concat(height, ".jpg"));
